 ...
 */
function piTaskPorts(): TaskMutationPorts {
	return {
		fileStore: {
...
export async function executeCodewikiTask(
	...
	if (input.action === "create") {
		...
		const result = await createCodewikiTasks(project, input.tasks, piTaskPorts());
		const details = {
	...
	if (input.action === "close") {
		...
		await cancelCodewikiTask(project, existingTask.id, piTaskPorts(), input.summary);
		changed = true;
		latestTask = (await readRoadmapTask(project, existingTask.id))!;
	} else {
		if (hasCodewikiTaskPatchChanges(input.patch)) {
			const patchResult = await patchCodewikiTask(project, existingTask.id, input.patch, piTaskPorts());
		...
		if (input.evidence) {
			if (
				...
				await appendTaskEvidence(project, latestTask.id, input.evidence, piTaskPorts());
				evidenceRecorded = true;
			}
		}
	}

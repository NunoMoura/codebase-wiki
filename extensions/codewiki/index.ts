...
import {
	...
	codewikiHeartbeatToolInputSchema,
...
async function executeCodewikiHeartbeat(
	project: WikiProject,
	ctx: ExtensionContext,
	input: HeartbeatToolInput,
): Promise<{ summary: string; mode: HeartbeatMode; budget: HeartbeatBudget; cycles: Array<Record<string, unknown>>; stop: Record<string, unknown>; policy: Record<string, unknown>; bounded_context: Record<string, unknown> }> {
...
function buildTaskVerifierPrompt(
	...
	return [
		...
		"Use only read-only inspection. Start from the compact task packet, expand only linked specs/evidence/code needed for the verdict, and avoid loading unrelated wiki history.",

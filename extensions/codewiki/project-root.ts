...
const DISCOVERY_EXCLUDED_DIRS = new Set([".git", ".hg", ".svn", ".pi", "node_modules", "dist", "build", "coverage", ".next", ".turbo"]);
...
export async function findWikiRootsBelow(startDir: string, options: { maxDepth?: number; maxResults?: number } = {}): Promise<string[]> {
...
async function hasAnyPath(root: string, relativePaths: readonly string[]): Promise<boolean> {
  for (const relativePath of relativePaths) {

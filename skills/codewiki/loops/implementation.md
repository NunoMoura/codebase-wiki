...
- Implementation/task-close validation must be performed from fresh validator context with clean-worktree and checked-SHA evidence; otherwise closure blocks. Use session handoff to start that validator context or worker process when policy/session budget allows.

...
def load_architecture_manifest() -> dict[str, Any]:
    path = SPECS_ROOT / "system" / "architecture.json"
    if not path.exists():
        return {"version": 1, "components": [], "flows": []}
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        return {"version": 1, "components": [], "flows": [], "issues": [{"severity": "error", "kind": "invalid-architecture-json", "path": path.relative_to(ROOT).as_posix(), "message": "Architecture manifest is not valid JSON."}]}
    return data if isinstance(data, dict) else {"version": 1, "components": [], "flows": []}


def architecture_validation_issues(manifest: dict[str, Any], docs_by_path: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    flows = manifest.get("flows", []) if isinstance(manifest.get("flows"), list) else []
    component_ids = {str(item.get("id", "")).strip() for item in components if isinstance(item, dict)}
    referenced_flow_paths = {str(item.get("path", "")).strip() for item in flows if isinstance(item, dict) and str(item.get("path", "")).strip()}
    component_paths = {str(item.get("path", "")).strip() for item in components if isinstance(item, dict) and str(item.get("path", "")).strip()}
    for component in components:
        ...
        if path and path not in docs_by_path:
            issues.append({"severity": "warning", "kind": "missing-component-doc", "path": path, "message": f"Architecture component {component_id} references a missing component doc."})
    for flow in flows:
        if not isinstance(flow, dict):
            continue
        flow_id = str(flow.get("id", "")).strip()
        source = str(flow.get("from", "")).strip()
        target = str(flow.get("to", "")).strip()
        path = str(flow.get("path", "")).strip()
        if source not in component_ids or target not in component_ids:
            issues.append({"severity": "warning", "kind": "unknown-architecture-component", "path": path or ".wiki/knowledge/system/architecture.json", "message": f"Architecture flow {flow_id} references unknown component(s)."})
        if path and path not in docs_by_path:
            issues.append({"severity": "warning", "kind": "missing-flow-doc", "path": path, "message": f"Architecture flow {flow_id} references a missing flow doc."})
    for path, doc in docs_by_path.items():
        if "/system/components/" in path and path not in component_paths:
            issues.append({"severity": "warning", "kind": "orphan-component-doc", "path": path, "message": "System component doc is not referenced by architecture.json."})
        if "/system/flows/" in path and path not in referenced_flow_paths and path.endswith(".md"):
            issues.append({"severity": "warning", "kind": "orphan-flow-doc", "path": path, "message": "System flow doc is not referenced by architecture.json."})
    return issues


def mermaid_node_id(component_id: str) -> str:
    return "n_" + "".join(ch if ch.isalnum() else "_" for ch in component_id)


def render_architecture_mermaid(architecture_view: dict[str, Any]) -> str:
    lines = ["%% GENERATED by CodeWiki. Do not hand-edit. Source: .wiki/knowledge/system/architecture.json", "flowchart TD"]
    for component in architecture_view.get("components", []):
        component_id = str(component.get("id", "")).strip()
        label = str(component.get("label") or component.get("title") or component_id).replace('"', "'")
        lines.append(f'  {mermaid_node_id(component_id)}["{label}"]')
    for flow in architecture_view.get("flows", []):
        source = str(flow.get("from", "")).strip()
        target = str(flow.get("to", "")).strip()
        label = str(flow.get("label") or flow.get("kind") or "flow").replace('"', "'")
        if source and target:
            lines.append(f'  {mermaid_node_id(source)} -->|"{label}"| {mermaid_node_id(target)}')
    return "\n".join(lines).rstrip() + "\n"
...
def build_system_architecture_view(docs: list[dict[str, Any]], graph: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in manifest.get("components", []) if isinstance(manifest.get("components"), list) else []:
        ...
        components.append({
            ...
            "label": str(item.get("label") or doc.get("title") or item.get("id", "")).strip(),
            "path": path,
            "summary": str(doc.get("summary") or item.get("summary", "")).strip(),
            "code_paths": [str(value).strip() for value in item.get("code_paths", doc.get("code_paths", [])) if str(value).strip()] if isinstance(item.get("code_paths", doc.get("code_paths", [])), list) else [],
            "depends_on": [str(value).strip() for value in item.get("depends_on", []) if str(value).strip()] if isinstance(item.get("depends_on"), list) else [],
            "doc_revision": compact_revision_digest(doc.get("revision", {})),
        })
    flows = []
    for item in manifest.get("flows", []) if isinstance(manifest.get("flows"), list) else []:
        if not isinstance(item, dict):
            continue
        path = str(item.get("path", "")).strip()
        doc = docs_by_path.get(path, {})
        flows.append({
            ...
            "kind": str(item.get("kind", "flow")).strip(),
    ...
    if not components:
        ...
        components = [{"id": str(doc.get("id", doc.get("path"))), "label": doc.get("title"), "path": doc.get("path"), "summary": doc.get("summary"), "code_paths": doc.get("code_paths", []), "depends_on": [], "doc_revision": compact_revision_digest(doc.get("revision", {}))} for doc in sorted(system_docs, key=lambda item: str(item.get("path", "")))[:80]]
    payload = {"source": ".wiki/knowledge/system/architecture.json", "manifest_revision": {"digest": canonical_digest(manifest), "git": compact_git_anchor([".wiki/knowledge/system/architecture.json"])}, "components": components, "flows": flows, "validation": {"issues": architecture_validation_issues(manifest, docs_by_path)}, "graph_revision": compact_graph_revision(graph)}
...
def write_v2_views(docs: list[dict[str, Any]], research_collections: list[dict[str, Any]], graph: dict[str, Any], roadmap_items: list[dict[str, Any]], lint_report: dict[str, Any], roadmap_state: dict[str, Any], status_state: dict[str, Any], events: list[dict[str, Any]]) -> None:
    ...
    write_view("system/architecture.json", architecture_view)
    (VIEWS_ROOT / "system" / "architecture.mmd").parent.mkdir(parents=True, exist_ok=True)
    (VIEWS_ROOT / "system" / "architecture.mmd").write_text(render_architecture_mermaid(architecture_view), encoding="utf-8")
